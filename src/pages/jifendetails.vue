<template>
  <div class="details">
    ...
    <div class="bottom">
      ...
      <div class="balance">账户积分：{{ pro.user_jifen }}</div>
    ...
    <mt-popup v-model="popupVisible">
      <div class="msg-box">
        ...
        <div class="msg-links">
          <router-link to="" class="msg-link">立即提货</router-link>
          <router-link to="" class="msg-link">猜奇偶</router-link>
          <router-link to="" class="msg-link">猜红包</router-link>
        </div>
      </div>
    </mt-popup>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      buyone () {
        var _this = this;
        if (parseFloat(_this.pro.user_jifen) < parseFloat(_this.pro.price_yh)) {
          _this.$toast({
            message: '积分不足',
            duration: 1000,
          });
      ...
      buyClick(){
        ...
        yuanAjax(_this, 'Api/Payment/prepay', {
          ...
          if (res.status == 1) {
            _this.$router.push({path: `/pay?pid=${_this.$route.query.id}&num=${_this.buynum}&paytype=jifen`});
          } else {
            _this.$toast({
              message: res.err,
              duration: 1000,
