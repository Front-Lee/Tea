...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      buyone () {
        var _this = this;
        if (parseFloat(_this.pro.user_balance) < parseFloat(_this.pro.price_yh)) {
          _this.$toast({
            message: '请先充值',
            duration: 1000,
          });
      ...
      buyClick(){
        ...
        yuanAjax(_this, 'Api/Payment/prepay', {
          ...
          if (res.status == 1) {
            _this.$router.push({path: `/pay?pid=${_this.$route.query.id}&num=${_this.buynum}&paytype=balance`});
          } else {
            _this.$toast({
              message: res.err,
              duration: 1000,
